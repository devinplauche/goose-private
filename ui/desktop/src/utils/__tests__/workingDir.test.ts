...
describe('resolveWorkingDir', () => {
  it('uses the configured external backend directory when present', () => {
    expect(resolveWorkingDir(' /home/warmachine ', 'C:\\Users\\warmachine', 'C:\\Users\\warmachine')).toBe(
      '/home/warmachine'
    );
    expect(resolveWorkingDir(' ', 'C:\\work', 'C:\\Users\\warmachine')).toBe('C:\\work');
    expect(resolveWorkingDir(undefined, undefined, 'C:\\Users\\warmachine')).toBe('C:\\Users\\warmachine');
  });
});
...
describe('getEffectiveWorkingDir', () => {
  ...
  const mockWindow = (externalBackend: boolean, boundUrl: string, source = 'settings') => {
    appConfigGetMock.mockImplementation((key: string) => {
      if (key === 'WARMACHINE_EXTERNAL_BACKEND') return externalBackend;
      if (key === 'WARMACHINE_EXTERNAL_BACKEND_URL') return boundUrl;
      if (key === 'WARMACHINE_EXTERNAL_BACKEND_SOURCE') return source;
      if (key === 'WARMACHINE_WORKING_DIR') return '/Users/johannes/home/workspace';
      return undefined;
    });
  };
  ...
  it('prefers the configured remote directory when bound to the matching external backend', async () => {
    ...
    getSettingMock.mockResolvedValue({
      ...
      workingDir: ' /home/warmachine/workspace ',
    });
    await expect(getEffectiveWorkingDir()).resolves.toBe('/home/warmachine/workspace');
  });
  ...
  it('ignores the remote directory when the window is bound to the local backend', async () => {
    mockWindow(false, '');
    getSettingMock.mockResolvedValue({ enabled: true, workingDir: '/home/warmachine/workspace' });
    await expect(getEffectiveWorkingDir()).resolves.toBe('/Users/johannes/home/workspace');
  });
  ...
  it('falls back to the remembered directory when the external backend is disabled', async () => {
    mockWindow(true, 'http://remote:3000');
    getSettingMock.mockResolvedValue({ enabled: false, workingDir: '/home/warmachine/workspace' });
    await expect(getEffectiveWorkingDir()).resolves.toBe('/Users/johannes/home/workspace');
  });
