#!/usr/bin/env bash
# run-benchmarks.sh - Script to run warmachine benchmarks across multiple provider:model pairs
...
function show_usage() {
  ...
  echo "  -t, --toolshim           Enable toolshim mode by setting WARMACHINE_TOOLSHIM=1"
  echo "  -m, --toolshim-model     Set the toolshim model (sets WARMACHINE_TOOLSHIM_MODEL)"
...
WARMACHINE_CMD="warmachine"
if [ "$DEBUG_MODE" = true ]; then
  if [ -f "./target/debug/warmachine" ]; then
    WARMACHINE_CMD="./target/debug/warmachine"
    echo "Using debug binary: $WARMACHINE_CMD"
  else
    echo "Warning: Debug binary not found at ./target/debug/warmachine. Falling back to system-installed warmachine."
  fi
else
  if [ -f "./target/release/warmachine" ]; then
    WARMACHINE_CMD="./target/release/warmachine"
    echo "Using release binary: $WARMACHINE_CMD"
  else
    echo "Warning: Release binary not found at ./target/release/warmachine. Falling back to system-installed warmachine."
  fi
fi
...
for ((i=0; i<$COUNT; i++)); do
  ...
  export WARMACHINE_PROVIDER="$provider"
  export WARMACHINE_MODEL="$model"
  
  # Set toolshim environment variables if enabled
  if [ "$TOOLSHIM" = true ]; then
    export WARMACHINE_TOOLSHIM=1
    if [[ -n "$TOOLSHIM_MODEL" ]]; then
      export WARMACHINE_TOOLSHIM_OLLAMA_MODEL="$TOOLSHIM_MODEL"
    fi
  fi
  ...
  if $WARMACHINE_CMD bench --suites "$SUITES" --output "$OUTPUT_FILE" --format json; then
