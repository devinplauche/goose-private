#!/usr/bin/env node
/**
 * Generates TypeScript types + Zod validators for WarMachine custom extension methods.
 *
 * Usage:
...
export default async function main() {
  ...
  console.log(`\nGenerated WarMachine extension schema in ${OUTPUT_DIR}`);
}
...
async function postProcessIndex(meta: {
  ...
  const methodConstants = await prettier.format(
    `
export const WARMACHINE_EXT_METHODS = ${JSON.stringify(meta.methods, null, 2)} as const;

export type GooseExtMethod = (typeof WARMACHINE_EXT_METHODS)[number];

export const WARMACHINE_EXT_NOTIFICATIONS = ${JSON.stringify(meta.notifications ?? [], null, 2)} as const;

export type GooseExtNotification = (typeof WARMACHINE_EXT_NOTIFICATIONS)[number];

export const WARMACHINE_EXT_AGENT_REQUESTS = ${JSON.stringify(meta.agentRequests ?? [], null, 2)} as const;
