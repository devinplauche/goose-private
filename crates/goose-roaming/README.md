...
Peer-to-peer transport for warmachine agents, built on
[iroh](https://iroh.computer) (QUIC, using iroh's public relays for NAT
traversal).

It lets a warmachine agent accept connections from a remote ACP client (another
warmachine, or any other ACP client) that drives it, and lets a client
...
This crate is a **standalone library** with no dependency on `warmachine` core (so
...
the transport to warmachine's agent machinery lives in `goose-cli` behind an optional
...
The host runs warmachine's real ACP server; the connecting side is an ACP client.
...
- You **swap cards** and each side **accepts** the other's key. A connection
  ...
  gets warmachine's full ACP surface. A leaked card lets no one in; there is no bearer
  token that works by possession.

## Concepts

- **`ConnectionCard`** — the shareable, non-secret identity + reachability string
  (`warmachine+roam://…`). Encodes public key + relay URLs; exposes `fingerprint()`.
...
An iroh bidirectional stream is the byte transport for warmachine's existing
transport-agnostic ACP `serve` / `ByteStreams` seam, so hosting reuses the ACP
server and the client reuses the ACP client.

## CLI

Exposed via `warmachine roam` (in `goose-cli`, feature `roaming`):
...
relays bridge them. On **each** machine run `warmachine roam id` and send the printed
`warmachine+roam://…` card to the other out of band (paste it in chat, etc.).

On **machine A** (the host): `warmachine roam peers accept '<B's card>'` then
`warmachine roam share` (optionally `--cwd <dir>`; it defaults to where `share`
started, and the connector's own path is always ignored). On **machine B**:
`warmachine roam peers add '<A's card>' boxA`, then either drive A interactively with
`warmachine roam connect boxA` (a prompt that runs on A's agent — its tools, files,
shell), hand it a one-shot task with `warmachine roam delegate boxA "what is 2+2?"`,
or `warmachine roam delegate boxA --list-sessions` / `--session <id> "<task>"` to
...
`warmachine roam connections` shows who connected. If session creation hangs on
macOS, prefix with `WARMACHINE_DISABLE_KEYRING=1`.
...
warmachine's real `serve`). `connect` is a thin ACP client UI — not a provider
...
**Acceptance is all-or-nothing.** An accepted peer gets warmachine's full ACP
...
(`roam-delegate`) documents how to call `warmachine roam delegate <peer> "<task>"` via
...
`warmachine roam share` agent — iroh compiled to wasm runs *inside the browser tab*,
