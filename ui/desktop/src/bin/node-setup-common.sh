...
if [ -n "${WARMACHINE_PATH_ROOT:-}" ]; then
    RESOLVED_WARMACHINE_CONFIG_DIR="${WARMACHINE_PATH_ROOT}/config"
elif [ -n "${WARMACHINE_CONFIG_DIR:-}" ]; then
    log "WARMACHINE_CONFIG_DIR is deprecated for desktop shims; prefer WARMACHINE_PATH_ROOT."
    RESOLVED_WARMACHINE_CONFIG_DIR="${WARMACHINE_CONFIG_DIR}"
else
    RESOLVED_WARMACHINE_CONFIG_DIR="${HOME}/.config/warmachine"
fi
MCP_HERMIT_DIR="${RESOLVED_WARMACHINE_CONFIG_DIR}/mcp-hermit"
mkdir -p "${RESOLVED_WARMACHINE_CONFIG_DIR}"
HERMIT_SETUP_LOCK_DIR="${RESOLVED_WARMACHINE_CONFIG_DIR}/.mcp-hermit-setup.lock"
...
CLEANUP_MARKER="${RESOLVED_WARMACHINE_CONFIG_DIR}/.mcp-hermit-cleanup-v1"
...
log "Checking for WARMACHINE_NPM_REGISTRY and WARMACHINE_NPM_CERT environment variables for custom npm registry setup..."
# Check if WARMACHINE_NPM_REGISTRY is set and accessible
if [ -n "${WARMACHINE_NPM_REGISTRY:-}" ] && curl -s --head --fail "${WARMACHINE_NPM_REGISTRY}" > /dev/null; then
    log "Checking custom warmachine registry availability: ${WARMACHINE_NPM_REGISTRY}"
    log "${WARMACHINE_NPM_REGISTRY} is accessible. Using it for npm registry."
    export NPM_CONFIG_REGISTRY="${WARMACHINE_NPM_REGISTRY}"

    # Check if WARMACHINE_NPM_CERT is set and accessible
    if [ -n "${WARMACHINE_NPM_CERT:-}" ] && curl -s --head --fail "${WARMACHINE_NPM_CERT}" > /dev/null; then
        log "Downloading certificate from: ${WARMACHINE_NPM_CERT}"
        curl -sSL -o "${MCP_HERMIT_DIR}/cert.pem" "${WARMACHINE_NPM_CERT}"
        if [ $? -eq 0 ]; then
        ...
        log "WARMACHINE_NPM_CERT is either not set or not accessible. Skipping certificate setup."
    fi

else
    log "WARMACHINE_NPM_REGISTRY is either not set or not accessible. Falling back to default npm registry."
