...
TEST_PROVIDER=${WARMACHINE_PROVIDER:-anthropic}
TEST_MODEL=${WARMACHINE_MODEL:-claude-haiku-4-5-20251001}
...
TMPFILE=$(mktemp)
(cd "$TESTDIR" && WARMACHINE_PROVIDER="$TEST_PROVIDER" WARMACHINE_MODEL="$TEST_MODEL" \
    "$WARMACHINE_BIN" run --recipe recipe.yaml 2>&1) | tee "$TMPFILE"
