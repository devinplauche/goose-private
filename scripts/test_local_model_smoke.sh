...
show_usage() {
  ...
  echo "      --ram-gb NUM         Override RAM passed to warmachine lm search"
  echo "      --instruction TEXT   Prompt to send to each model"
  echo "      --repo-prefix TEXT   Forwarded to warmachine lm search"
  echo "      --repo-suffix TEXT   Forwarded to warmachine lm search"
  echo "      --quant TEXT         Forwarded to warmachine lm search"
  ...
  echo ""
  echo "Environment:"
  echo "  WARMACHINE_BIN                Optional warmachine binary path"
...
if [[ -z "${SKIP_BUILD:-}" && -z "${WARMACHINE_BIN:-}" ]]; then
  echo "Building warmachine..."
  (cd "$REPO_ROOT" && cargo build -p goose-cli --features local-inference --bin warmachine)
  echo ""
fi

WARMACHINE_BIN="${WARMACHINE_BIN:-$REPO_ROOT/target/debug/warmachine}"
if [[ ! -x "$WARMACHINE_BIN" ]]; then
  echo "Error: warmachine binary not found or not executable: $WARMACHINE_BIN"
  exit 1
fi
...
"$WARMACHINE_BIN" lm list | awk 'NR > 2 && $4 == "✓" { print $1 }' > "$EXISTING_MODELS_FILE"
...
cleanup_temp_models() {
  ...
  for model_id in "${TEMP_MODELS[@]}"; do
    if ! "$WARMACHINE_BIN" lm delete "$model_id" >/dev/null 2>&1; then
      cleanup_failed=true
    fi
  done
...
if [[ -n "$MODEL_LIST" ]]; then
  ...
  "$WARMACHINE_BIN" "${SEARCH_ARGS[@]}" > "$SEARCH_JSON"
...
download_once() {
  ...
  if [[ -n "$cache_root" ]]; then
    HF_HUB_CACHE="$cache_root/hub" \
      ...
      "$WARMACHINE_BIN" lm download "$download_id"
  else
    "$WARMACHINE_BIN" lm download "$download_id"
  fi
}
...
run_model() {
  ...
  if [[ "$RUN_TIMEOUT" -eq 0 ]]; then
    WARMACHINE_MODE=auto WARMACHINE_PROVIDER=local WARMACHINE_MODEL="$model_id" \
      "$WARMACHINE_BIN" run --no-profile --text "$INSTRUCTION" 2>&1 | tee "$log_file"
    return "${PIPESTATUS[0]}"
  fi
  ...
  ' \
    "$RUN_TIMEOUT" \
    env WARMACHINE_MODE=auto WARMACHINE_PROVIDER=local WARMACHINE_MODEL="$model_id" \
    "$WARMACHINE_BIN" run --no-profile --text "$INSTRUCTION" 2>&1 | tee "$log_file"
  return "${PIPESTATUS[0]}"
}
...
for row in "${MODELS[@]}"; do
  ...
  if [[ "$downloaded" = true ]]; then
    ...
    if [[ ! -s "$run_log" ]]; then
      ...
      echo "WarMachine reported an error for $model_id"
      echo "  $error_summary"
  ...
  if [[ "$KEEP_DOWNLOADS" = false && "$downloaded" = true && "$existed_before" = false ]]; then
    if "$WARMACHINE_BIN" lm delete "$model_id" 2>&1 | tee "$delete_log"; then
