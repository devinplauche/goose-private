...
export async function generateDeepLink(recipe: Recipe): Promise<string> {
  ...
  return `warmachine://recipe?config=${encoded}`;
}
...
export async function parseDeeplink(deeplink: string): Promise<Recipe | null> {
  try {
    ...
    if (!cleanLink.startsWith('warmachine://recipe?config=')) {
      throw new Error('Invalid deeplink format. Expected: warmachine://recipe?config=...');
    }

    const recipeEncoded = cleanLink.replace('warmachine://recipe?config=', '');
