...
show_usage() {
  ...
  echo ""
  echo "Environment:"
  echo "  OPENROUTER_API_KEY       Required by warmachine's OpenRouter provider"
...
if [[ -z "${SKIP_BUILD:-}" && -z "${WARMACHINE_BIN:-}" ]]; then
  ...
  (cd "$REPO_ROOT" && cargo build --bin warmachine)
  echo ""
fi
...
def get_weather(
    ...
    return f"WARMACHINE_TOOL_CALL_OK: The weather in {location} is 68 F and clear."
EOF

cat > "$TESTDIR/recipe.yaml" << 'EOF'
title: OpenRouter Tool Call Test
description: Test a model can call a simple MCP tool through warmachine
...
run_model() {
  ...
  if [[ "$RUN_TIMEOUT" -eq 0 ]]; then
    WARMACHINE_MODE=auto WARMACHINE_PROVIDER=openrouter WARMACHINE_MODEL="$model" \
      "$WARMACHINE_BIN" run --no-profile --max-turns 4 --recipe recipe.yaml
    return $?
  fi
  ...
  ' \
    "$RUN_TIMEOUT" \
    env WARMACHINE_MODE=auto WARMACHINE_PROVIDER=openrouter WARMACHINE_MODEL="$model" \
    "$WARMACHINE_BIN" run --no-profile --max-turns 4 --recipe recipe.yaml
}
...
for model in "${MODELS[@]}"; do
  ...
  if (cd "$TESTDIR" && run_model "$model" 2>&1) | tee "$log_file"; then
    ...
    echo "✗ WarMachine run failed for $model"
    if [[ "$run_status" -eq 124 ]]; then
      ...
      RESULTS+=("✗ $model - warmachine run failed")
    fi
    OVERALL_SUCCESS=false
  fi
