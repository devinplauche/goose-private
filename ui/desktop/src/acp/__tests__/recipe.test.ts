...
describe('ACP recipe helpers', () => {
  ...
  it('surfaces ACP JSON-RPC error messages', async () => {
    client.warmachine.recipesEncode_unstable.mockRejectedValue({
  ...
  it('prefers ACP JSON-RPC error data over generic messages', async () => {
    client.warmachine.recipesSave_unstable.mockRejectedValue({
      error: {
  ...
  it('prefers ACP JSON-RPC error data from Error instances', async () => {
    client.warmachine.recipesParse_unstable.mockRejectedValue(
  ...
  it('shares concurrent recipe list requests', async () => {
    ...
    client.warmachine.recipesList_unstable.mockResolvedValue({ recipes });

    const [first, second] = await Promise.all([listRecipes(), listRecipes()]);

    expect(client.warmachine.recipesList_unstable).toHaveBeenCalledTimes(1);
  ...
  it('fetches recipes again after a list request settles', async () => {
    ...
    expect(client.warmachine.recipesList_unstable).toHaveBeenCalledTimes(2);
  });
});
