# WarMachine Buzz automation

These tools connect issues in `aaif-goose/warmachine` to the public WarMachine Buzz
...
- WarMachine CLI with a configured model provider
...
```text
$WARMACHINE_BUZZ_HOME/github-manager
```

`WARMACHINE_BUZZ_HOME` defaults to `$XDG_CONFIG_HOME/warmachine/buzz`, or
`~/.config/warmachine/buzz` when `XDG_CONFIG_HOME` is not set. The identity directory
...
```sh
export BUZZ_PRIVATE_KEY="$(cat ~/.config/warmachine/buzz/github-manager/private-key.nsec)"
...
- Mic as a member
- lifei as a member
    - Lifei warmachine agent as their bot
...
Use `--repo owner/repo` for a repository other than `aaif-goose/warmachine`. A full
...
queue entry can contain a WarMachine issue or pull request URL, or just `#<issue
...
keys, interests, and assignment capacity so a WarMachine recipe can select an owner.
...
This WarMachine recipe manages the full Inbox loop:
...
The dedicated machine must stay awake and have working WarMachine, `gh`, and Buzz
CLI configuration. This runner does not require WarMachine's scheduler.
...
```text
~/.config/warmachine/buzz/github-manager
```
...
install and authenticate `gh`, install and configure WarMachine, and install Buzz
