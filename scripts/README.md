# WarMachine Benchmark Scripts

This directory contains scripts for running and analyzing WarMachine benchmarks.

## run-benchmarks.sh

This script runs WarMachine benchmarks across multiple provider:model pairs and analyzes the results.

### Prerequisites

- WarMachine CLI must be built or installed
...
3. For each provider:model pair:
   - Sets the `WARMACHINE_PROVIDER` and `WARMACHINE_MODEL` environment variables
