...
warmachine is open source, and code is only one way to contribute. Reporting a problem, reproducing it, sharing domain knowledge, shaping the design, implementing the solution, and verifying the result are all valuable work.

We organize this work on the public [WarMachine Issues board](https://github.com/orgs/aaif-goose/projects/1). The issue is the main record of a contribution, from the first report through design, implementation, and verification.
...
Every open issue is tracked on the [WarMachine Issues board](https://github.com/orgs/aaif-goose/projects/1):
...
Do not begin implementation or open a pull request until the issue has reached **Ready** on the WarMachine Issues board.
...
you like warmachine of course and use that. One thing to watch out for is LLM eagerness. They like to please and
are in a hurry. 
...
warmachine includes Rust binaries alongside an electron app for the GUI.
...
First let's compile warmachine and try it out
Since warmachine requires Hermit for managing dependencies, let's activate hermit.
...
When that completes, debug builds of the binaries are available, including the warmachine CLI:

```
./target/debug/warmachine --help
```

For first-time setup, run the configure command:

```
./target/debug/warmachine configure
```

Once a connection to an LLM provider is working, start a session:

```
./target/debug/warmachine session
```
...
The app opens a window and displays first-time setup. After completing setup, warmachine is ready for use.
...
```
export WARMACHINE_SERVER__SECRET_KEY=test
cargo run --package goose-cli --bin warmachine -- serve --platform desktop --enable-scheduler --host 127.0.0.1 --port 3000
```

The `debug-ui` recipe connects to `http://127.0.0.1:3000` by default. If the
backend uses another port, set `WARMACHINE_PORT` when starting the UI, or set
`WARMACHINE_EXTERNAL_BACKEND_URL` to the backend's HTTP base URL.
...
2. This creates https://github.com/<your-username>/warmachine under your GitHub account.
3. Clone your fork (not the main repo):

```
git clone https://github.com/<your-username>/warmachine.git
cd warmachine
```
...
8. Open a Pull Request from your branch on your fork to aaif-goose/warmachine’s main branch.
...
You can change the provider warmachine points to via the `WARMACHINE_PROVIDER` env var. If you already
...
When testing changes or running multiple warmachine configurations, use `WARMACHINE_PATH_ROOT` to isolate your data:

```bash
# Test with a clean environment
export WARMACHINE_PATH_ROOT="/tmp/goose-test"
./target/debug/warmachine session

# Or for a single command
WARMACHINE_PATH_ROOT="/tmp/goose-dev" cargo run -p goose-cli -- session
```

This creates isolated `config/`, `data/`, and `state/` directories under the specified path, preventing your test sessions from affecting your main warmachine installation. See the [environment variables guide](./documentation/docs/guides/environment-variables.md#development--testing) for more details.

## Enable traces in warmachine with [locally hosted Langfuse](https://langfuse.com/docs/deployment/self-host)
...
- Set the environment variables so that warmachine can connect to the langfuse server:
...
There are numerous ways to be an open source contributor and contribute to warmachine. We're here to help you on your way! Here are some suggestions to get started. If you have any questions or need help, feel free to reach out to us on [Discord](https://discord.gg/n8R5VaWDAn).

- **Stars on GitHub:** If you resonate with our project and find it valuable, consider starring our warmachine on GitHub! 🌟
...
- **Give Feedback:** Have a feature you want to see or encounter an issue with warmachine, [click here to open an issue](https://github.com/aaif-goose/goose/issues/new/choose), [start a discussion](https://github.com/aaif-goose/goose/discussions) or tell us on Discord.
...
- **Spread the Word:** Help us reach more people by sharing warmachine's project, website, YouTube, and/or Twitter/X.
