...
**Status: sections 1–3 are done.** `cargo test -p warmachine` is down to the 4 `jsonwebtoken`
...
- `cargo build -p goose-cli` did not compile on this branch at all. `ActionRequiredData`
  ...
  arm, and the crate is not covered by `cargo test -p warmachine`. Fixed here, along with the
...
- The platform-extension prompt snapshot had been regenerated without the `code-mode`
  ...
  `cargo test -p warmachine`. Regenerated with the workspace feature set: it now differs from
...
Nine tests fail because of this: 7 in `crates/warmachine/tests/agent.rs`, 2 in
`crates/warmachine/tests/compaction.rs`. (The 4 `jsonwebtoken` failures in the lib and the 6 in
...
### `crates/warmachine/src/agents/agent.rs`
...
### `crates/warmachine/src/agents/reply_parts.rs`
...
### `crates/warmachine/tests/agent.rs`, `crates/warmachine/tests/compaction.rs`, `crates/warmachine/src/agents/execute_commands.rs`
...
- `crates/warmachine/src/providers/oauth.rs` — the `test_token_cache` rewrite is unrelated to
...
- `crates/warmachine/src/acp/server.rs` — `MessageContent::Error` as an agent message chunk, and
...
```bash
cargo test -p warmachine --test agent --test compaction
cargo test -p warmachine --lib agents::state_machine
```
...
cargo test -p warmachine --no-fail-fast
```
