...
export class DesktopFileAccess {
  ...
  private bindingForWindow(windowId: number): WorkingDirectoryBinding {
    ...
    if (!binding) {
      throw new Error('This window is not authorized to access .warmachinehints');
    }
    return binding;
  }
  ...
  async readGoosehints(windowId: number): Promise<FileReadResult> {
    ...
    try {
      ...
      if (metadata.isSymbolicLink()) {
        return failedRead(filePath, 'Refusing to read a symbolic link as .warmachinehints');
      }
      if (!metadata.isFile()) {
        return failedRead(filePath, '.warmachinehints is not a regular file');
      }

      const canonicalFilePath = await fs.realpath(filePath);
      if (path.dirname(canonicalFilePath) !== binding.path) {
        return failedRead(filePath, '.warmachinehints resolves outside the working directory');
      }
      ...
      try {
        ...
        if (openedMetadata.dev !== metadata.dev || openedMetadata.ino !== metadata.ino) {
          return failedRead(filePath, '.warmachinehints changed while it was being opened');
        }
        if (!(await this.bindingMatchesDirectory(binding))) {
          return failedRead(filePath, 'The working directory changed after it was authorized');
        }
      ...
      return failedRead(filePath, 'Unable to read .warmachinehints');
    }
  }
