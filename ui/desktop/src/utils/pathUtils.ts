...
export function sanitizeGoosePathRoot(env: { WARMACHINE_PATH_ROOT?: string }): string | undefined {
  const pathRoot = resolveGoosePathRoot(env.WARMACHINE_PATH_ROOT);
  if (pathRoot) {
    env.WARMACHINE_PATH_ROOT = pathRoot;
  } else {
    delete env.WARMACHINE_PATH_ROOT;
  }
  return pathRoot;
}
