...
export async function acpListSchedules(): Promise<ScheduledJobDto[]> {
  ...
  const listPromise = (async () => {
    ...
    const response = await client.warmachine.schedulesList_unstable({});
...
export async function acpCreateSchedule(
  ...
  try {
    const client = await getAcpClient();
    const response = await client.warmachine.schedulesCreate_unstable(request);
...
export async function acpDeleteSchedule(scheduleId: string): Promise<void> {
  try {
    const client = await getAcpClient();
    await client.warmachine.schedulesDelete_unstable({ scheduleId });
    clearInFlightScheduleReads();
  } catch (error) {
...
export async function acpListScheduleSessions(
  ...
  const listPromise = (async () => {
    const client = await getAcpClient();
    const response = await client.warmachine.schedulesSessionsList_unstable({ scheduleId, limit });
...
export async function acpRunScheduleNow(
  ...
  try {
    const client = await getAcpClient();
    const response = await client.warmachine.schedulesRunNow_unstable({ scheduleId });
    clearInFlightScheduleReads();
    return response;
  } catch (error) {
...
export async function acpPauseSchedule(scheduleId: string): Promise<void> {
  try {
    const client = await getAcpClient();
    await client.warmachine.schedulesPause_unstable({ scheduleId });
    clearInFlightScheduleReads();
  } catch (error) {
...
export async function acpUnpauseSchedule(scheduleId: string): Promise<void> {
  try {
    const client = await getAcpClient();
    await client.warmachine.schedulesUnpause_unstable({ scheduleId });
    clearInFlightScheduleReads();
  } catch (error) {
...
export async function acpUpdateSchedule(
  ...
  try {
    const client = await getAcpClient();
    const response = await client.warmachine.schedulesUpdate_unstable({ scheduleId, cron });
    clearInFlightScheduleReads();
    return response.job;
  } catch (error) {
...
export async function acpKillRunningJob(jobId: string): Promise<KillRunningJobResponse_unstable> {
  try {
    const client = await getAcpClient();
    const response = await client.warmachine.schedulesRunningJobKill_unstable({ jobId });
    clearInFlightScheduleReads();
    return response;
  } catch (error) {
...
export async function acpInspectRunningJob(
  ...
  try {
    const client = await getAcpClient();
    return await client.warmachine.schedulesRunningJobInspect_unstable({ jobId });
  } catch (error) {
