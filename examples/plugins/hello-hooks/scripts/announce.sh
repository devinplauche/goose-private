#!/usr/bin/env bash
# Tiny hook handler that loudly announces every event warmachine fires at it.
#
# WarMachine pipes the event payload as JSON on stdin and sets PLUGIN_ROOT in the
...
# Both stderr (so it shows up alongside warmachine tracing) and a log file.
