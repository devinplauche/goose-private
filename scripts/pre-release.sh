...
REPO="${WARMACHINE_GITHUB_REPO:-aaif-goose/warmachine}"
DEST="$HOME/Downloads"
TMPDIR=$(mktemp -d)
...
if ! curl -fSL --connect-timeout 10 -o "$TMPDIR/warmachine.zip" "$DOWNLOAD_URL" 2>/dev/null; then
    ...
    gh api "repos/$REPO/actions/artifacts/$ARTIFACT_ID/zip" > "$TMPDIR/warmachine.zip"
fi
echo "Done."

unzip -o -q "$TMPDIR/warmachine.zip" -d "$TMPDIR/extracted"
...
APP_NAME="WarMachine ${VERSION}.app"
rm -rf "$DEST/$APP_NAME"
